...
// Category labels (maps backend category values)
...
// Status colors and labels (maps both old and backend status values)
const STATUS_CONFIG: Record<string, { label: string; color: string }> = {
  open: { label: 'Open', color: C.primary },
  submitted: { label: 'Submitted', color: C.primary },
...
// Ticket type from query (adapted for supportRequests)
...
export default function SupportHistoryScreen() {
  ...
  // Using api.support.getMySupportRequests instead of supportTickets
  const supportResponse = useQuery(
    api.support.getMySupportRequests,
    userId ? { authUserId: userId } : 'skip'
  );

  // Transform backend response to match expected UI format
  const tickets = React.useMemo((): TicketWithPreview[] | undefined => {
    if (supportResponse === undefined) return undefined;
    if (!supportResponse.success) return [];
    return supportResponse.requests.map((r: any) => ({
      _id: r.requestId,
      category: r.category,
      status: r.status === 'resolved' ? 'replied' : r.status === 'submitted' ? 'open' : r.status,
      message: '', // Not available in response
      createdAt: r.createdAt,
      updatedAt: r.updatedAt || r.createdAt,
      lastMessage: null, // Thread messages not fetched here
      messageCount: 0,
      hasAdminReply: r.status === 'resolved' || r.status === 'in_review',
    }));
  }, [supportResponse]);
