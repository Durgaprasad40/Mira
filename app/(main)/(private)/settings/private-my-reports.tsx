...
// Report reason labels (maps both old keys and backend category values)
...
export default function PrivateMyReportsScreen() {
  ...
  // Query real reports from backend using api.support
  const backendResponse = useQuery(
  ...
  const reports = useMemo((): Report[] => {
    if (!backendResponse?.success || !backendResponse.requests) return [];
    return backendResponse.requests.map((r: any) => ({
      _id: r.requestId,
      reason: r.category, // Backend uses 'category' (e.g., harassment_stalking)
      status: mapBackendStatus(r.status),
      createdAt: r.createdAt,
      reportedUserName: r.relatedUser?.displayName,
      hasAttachments: false, // Not yet supported
    }));
  }, [backendResponse]);

  // Map backend ticket status to UI status
  // Backend values: 'submitted', 'in_review', 'resolved', 'closed'
  ...
  const isLoading = !isDemoMode && userId && backendResponse === undefined;
