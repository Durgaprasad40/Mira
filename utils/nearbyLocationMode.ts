 ...
 * - 'foreground': Only track location while app is open
 * - 'background': legacy compatibility value; shipped Phase-1 Nearby is foreground-only
 *
 * PERSISTENCE:
 ...
 *
 * FALLBACK BEHAVIOR:
 * - Old persisted background selections are normalized back to foreground
 * - effectiveMode reflects the shipped foreground-only behavior
 *
 * SAFETY:
...
export async function getPreferredLocationMode(): Promise<NearbyLocationMode> {
  try {
    ...
    if (mode === 'background') {
      await AsyncStorage.setItem(LOCATION_MODE_KEY, 'foreground');
    }
    return 'foreground';
  } catch {
    return 'foreground';
...
export async function getEffectiveLocationMode(): Promise<NearbyLocationMode> {
  try {
    ...
    if (mode === 'background') {
      await AsyncStorage.setItem(EFFECTIVE_MODE_KEY, 'foreground');
    }
    return 'foreground';
  } catch {
    return 'foreground';
 ...
 *
 * Shipped Phase-1 Nearby is foreground-only, so this requests foreground access only.
 */
