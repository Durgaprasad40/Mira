...
export const useDemoStore = create<DemoState>()((set, get) => ({
  ...
  saveDemoProfile: (userId, data) => {
    // LIVE MODE GUARD: saveDemoProfile is demo-only, NO-OP in live mode
    const isDemoMode = process.env.EXPO_PUBLIC_DEMO_MODE === 'true';
    if (!isDemoMode) {
      if (__DEV__) console.warn('[DemoStore] saveDemoProfile skipped (live mode)');
      return;
    }
  ...
  blockUser: (userId) => {
    ...
    const { useConfessionStore } = require('@/stores/confessionStore') as {
      useConfessionStore: {
        getState: () => {
          ...
          confessions: Array<{ id: string; userId: string; taggedUserId?: string }>;
          blockUser: (userId: string) => void;
        };
    ...
    for (const [confessionId, convoId] of Object.entries(confessionThreads)) {
      ...
      if (confession?.userId === userId || confession?.taggedUserId === userId) {
